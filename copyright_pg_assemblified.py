        ...
        program = [ 
            ...
            # Making X_words (a list of strings) from X (a string)
                ...
                ## Only IF X[curr] == " ": Append X[idx-k : idx-i] (from last blank to current blank = word) to X_words
                    Instr(7, 9, 2, 0, 0, "", 8, 0, 6, 1),       ## Step5  #7: Assign idx9 (idx-i/reg2) of idx 2 (X) to idx 8(reg1)
                ...
                ## Only IF  X[-1] != " " (if string not ending with blank): Append X[k:] (the last word) to X_words
                    Instr(7, -1, 2, 0, 0, "", 8, 0, 6, 0),      ## Step16  #7 take last elem of idx2 (X) into idx 8(reg1)
            ...
            # Take the first three nouns from X and hard-code the rest from the fill list
                ...
                ## FIRST IF curr madlibs_words is equal to "_"
                    Instr(7, 9, 3, 0, 0, "", 8, 0, 6, 1),       ## Step24  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(reg1)
                ...
                ## IF Both TRUE (Append from X_Words)
                    Instr(7, 9, 4, 0, 0, "", 8, 0, 6, 1),       ## Step29  #7: Assign idx 9 (idx-i) of idx 4 (X_words) to idx 8(reg1)
                    Instr(4, 5, 0, 0, 0, "", 0, 0, 6, 0),       ## Step30  #4: jump to +5

                ## IF only the former TRUE (Append from fill/consts)
                    Instr(7, 10, 7, 0, 0, "", 8, 0, 6, 1),      ## Step31  #7: Assign idx10 (idx-k) of idx 7 (fill) to idx 8(reg1)
            ...
            # Stringify the assembled_list into result
                ...
                ## Only IF idx-i == 0: Append assembled_list[0] to result
                    ...
                    Instr(7, 0, 5, 0, 0, 0, 6, 0, 6, 2),       ## Step43  #7: Take the first element (idx 0) of idx5(assembled_list) and set it to des(6:result)
                    Instr(1, 0, 0, 0, 1, "", 9, 0, 6, 0),       ## Step44  #1: Set 1 to idx 9(idx-i)
                    
                ## Append " " +  assembled_list[idx-i] to result
                    Instr(7, 9, 5, 0, 0, "", 8, 0, 6, 1),       ## Step45  #7: Take idx 9(idx-i) of idx5 (assembled_list) and set it to idx8(reg1)
        ...
        program = [
            ...
            # Hard-Code all blanks from the nouns list
                ...
                ## IF madlibs_words[curr] == "_"
                    Instr(7, 9, 3, 0, 0, "", 8, 0, 6, 1),       ## Step11  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(reg1)
                    ...
                    Instr(7, 10, 7, 0, 0, "", 8, 0, 6, 1),      ## Step14  #7: Assign idx10 (idx-k) of idx 7 (fill) to idx 8(reg1)
            ...
            # Stringify the assembled_list into result
                ...
                ## Only IF idx-i == 0: Append assembled_list[0] to result
                    ...
                    Instr(7, 0, 5, 0, 0, 0, 6, 0, 6, 2),       ## Step27  #7: Take the first element (idx 0) of idx5(assembled_list) and set it to des(6:result)
                    Instr(1, 0, 0, 0, 1, "", 9, 0, 6, 0),       ## Step28  #1: Set 1 to idx 9(idx-i)
                    
                ## Append " " +  assembled_list[idx-i] to result
                    Instr(7, 9, 5, 0, 0, "", 8, 0, 6, 1),       ## Step29  #7: Take idx 9(idx-i) of idx5 (assembled_list) and set it to idx8(reg1)
