...
def main():
    ...
    program = [ 
        #  Making madlibs_words (a list of strings) by spliting madlibs into words
            ...
            ## Only IF madlibs[curr] (char) == " ": Append madlibs[idx-k : idx-i] (from last blank to current blank = word) to madlibs_words
               Instr(7, 0, 9, 0, 8, 1),       ## Step0  #7: Assign idx9 (idx-i) of idx 0(madlibs) to idx 8(bot1)
               Instr(3, 8, " ", 0, 8, 0),     ## Step1  #3: Compare idx 8(bot1) == " " and assign result to idx 8(bot1)
               Instr(4, 1, 5, 8, 0, 1),       ## Step2  #4: Cond jump to +1/+5 if true/false

               Instr(6, 0, 10, 9, 8, 0),      ## Step3  #6: Assign idx 10 to 9 of idx 0 (madlibs) to idx 8 (bot1)
            ...
            ## Determine whether or not to iterate over again depending idx-i< len(madlibs_len)
               Instr(3, 9, mad_len, 2, 8, 0), ## Step8  #3: Check if idx 9 (idx-i) < madlibs_len, and assign result to idx 8(bot1)
            ...
            ## Only IF last elem of madlibs != " " (if string not ending with blank): Append madlibs[k:] (the last word) to madlibs_words
               Instr(7, 0, -1, 0, 8, 0),      ## Step10  #7: take last elem of idx 0(madlibs) into idx 8(bot1)
               Instr(3, 8, " ", 1, 8, 0),     ## Step11  #3: Compare idx8(bot1) != " ", assign it to idx 8(bot1)
               Instr(4, 1, 3, 8, 0, 1),       ## Step12  #4: Cond jump to +1/+3 if true/false

               Instr(6, 0, 10, 0, 8, 1),      ## Step13  #6: Assign idx 10 till end of idx 0(madlibs) to idx 8 (bot1)
               Instr(8, 8, 0, 0, 3, 0),       ## Step14  #8: append idx8 (bot1) to idx 3(madlibs)
        ...
        # Make a fill list from hard-coded nouns 1 and 2
               ...
               Instr(7, 1, 3, 0, 8, 0),    ## Step17  #7: Set index 3 of idx1 (nouns) to idx 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step18  #8: Append hard-coded noun1/2, idx 8(bot1), to idx 7(fill)
               Instr(7, 1, 4, 0, 8, 0),    ## Step19  #7: Set index 4 of idx1 (nouns) to idx 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step20  #8: Append hard-coded noun2/2, idx 8(bot1), to idx 7(fill)
        ...
        # Making X_words (a list of strings) from X (a string)
            ...
            ## Only IF X[curr] == " ": Append X[idx-k : idx-i] (from last blank to current blank = word) to X_words
               Instr(7, 2, 9, 0, 8, 1),     ## Step21  #7: Assign idx9 (idx-i) of idx 2 (X) to idx 8(bot1)
               Instr(3, 8, " ", 0, 8, 0),   ## Step22  #3: Compare idx 8(bot1) and " " and assign result to idx 8(bot1)
               Instr(4, 1, 5, 8, 0, 1),      ## Step23  #4: Cond jump to +1/+5 if true/false

               Instr(6, 2, 10, 9, 8, 0),    ## Step24  #6: Assign idx10 (idx-k) to idx9 (idx-i) of idx 2 (X) to idx 8 (bot1)
               Instr(8, 8, 0, 0, 4, 0),      ## Step25  #8: Append idx8 (bot1) to idx 4 (X_words)
            ...
            ## Determine whether or not to iterate over again depending idx-i< len(X)
               Instr(3, 9, X_len, 2, 8, 0), ## Step29  #3: Compare idx 9 (idx-i) < len(X) and assign result to idx 8(bot1)
            ...
            ## Only IF  X[-1] != " " (if string not ending with blank): Append X[k:] (the last word) to X_words
               Instr(7, 2, -1, 0, 8, 0),     ## Step31  #7 take last elem of idx2 (X) into idx 8(bot1)
               Instr(3, 8, " ", 1, 8, 0),   ## Step32  #3: Compare idx8(bot1) != " ", assign it to idx 8(bot1)
               Instr(4, 1, 3, 8, 0, 1),      ## Step33  #4: Cond jump to +1/+3 if true/false

               Instr(6, 2, 10, 0, 8, 1),     ## Step34  #6: Assign idx 10(idx-k) till end of idx 2(X) to idx 8 (bot1)
        ...
        # Take the first three nouns from X and hard-code the rest from the fill list
            ...
            ## FIRST IF curr madlibs_words is equal to "_"
               Instr(7, 3, 9, 0, 8, 1),          ## Step38  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(bot1)
               Instr(3, 8, "_", 0, 8, 0),        ## Step39  #3: Compare idx 8(bot1) and "_" and assign result to idx 8(bot1)
            ...
            ## SECOND IF index of madlibs_words is less than fill_upto (upto idx of third fill)
               Instr(3, 9, fill_upto, 2, 8, 0),  ## Step41  #3: Compare idx 9(idx-i) < fill_upto (10 for now) and assign result to idx 8(bot1)
               Instr(4, 1, 3, 8, 0, 1),           ## Step42  #4: Cond jump to +1/+3 if true/false

            ## IF Both TRUE (Append from X_Words)
               Instr(7, 4, 9, 0, 8, 1),          ## Step43  #7: Assign idx 9 (idx-i) of idx 4 (X_words) to idx 8(bot1)
               Instr(4, 5, 0, 0, 0, 0),            ## Step44  #4: jump to +5

            ## IF only the former TRUE (Append from fill/consts)
               Instr(7, 7, 10, 0, 8, 1),          ## Step45  #7: Assign idx10 (idx-k) of idx 7 (fill) to idx 8(bot1)
               Instr(2, 1, 0, 0, 10, 0),           ## Step46  #2: add 1 to idx 10 (idx-k)
               Instr(4, 2, 0, 0, 0, 0),            ## Step47  #4: jump to +2

            ## ELSE (Append from madlibs_words)
               Instr(7, 3, 9, 0, 8, 1),          ## Step48  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(bot1)

            ## APPEND and INCREMENT
               Instr(8, 8, 0, 0, 5, 0),           ## Step49  #8: append idx8 (bot1) to idx5 (assembled_list)
               Instr(2, 1, 0, 0, 9, 0),           ## Step50  #2: add 1 to idx 9 (idx-i)
                
            ## CHECK IF ITERATE OR NEXT
               Instr(5, 4, 0, 0, 8, 0),           ## Step51  #9: Measure a length of index4(X_words) and assign it to idx 8(bot1)
               Instr(3, 9, 8, 2, 8, 1),         ## Step52  #3: Compare idx 9(idx-i) < idx 8(bot1) and assign result to idx 8(bot1)
    ...
    program = [
        ...
        # Make a fill list by appending hard-coded nouns 1 - 5
               ...
               Instr(7, 1, 0, 0, 8, 0),    ## Step17  #7: Set index 0 of idx1 (nouns) to 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step18  #14: Append hard-coded noun1/5 to idx6 (fill)
               Instr(7, 1, 1, 0, 8, 0),    ## Step19  #7: Set index 0 of idx1 (nouns) to 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step20  #14: Append hard-coded noun2/5 to idx6 (fil
               Instr(7, 1, 2, 0, 8, 0),    ## Step21  #7: Set index 0 of idx1 (nouns) to 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step22  #14: Append hard-coded noun3/5 to idx6 (fil
               Instr(7, 1, 3, 0, 8, 0),    ## Step23  #7: Set index 0 of idx1 (nouns) to 8 (bot1)
               Instr(8, 8, 0, 0, 7, 0),    ## Step24  #14: Append hard-coded noun4/5 to idx6 (fil
               Instr(7, 1, 4, 0, 8, 0),    ## Step25  #7: Set index 0 of idx1 (nouns) to 8 (bot1)
        ...
        # Hard-Code all blanks from the nouns list
            ...
            ## IF madlibs_words[curr] == "_"
               Instr(7, 3, 9, 0, 8, 1),    ## Step27  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(bot1)
               Instr(3, 8, "_", 0, 8, 0),  ## Step28  #3: Compare idx 10(bot1) and "_" and assign result to idx 10(bot1)
               Instr(4, 1, 4, 8, 0, 1),     ## Step29  #4: Cond jump to +1/+4 if true/false

               ## TRUE: Append from fill[idx-k] to assembled_list
               Instr(7, 7, 10, 0, 8, 1),    ## Step30  #7: Assign idx10 (idx-k) of idx 7 (fill) to idx 8(bot1)
               ...
               Instr(7, 3, 9, 0, 8, 1),    ## Step33  #7: Assign idx9 (idx-i) of idx 3 (madlibs_words) to idx 8(bot1)
                
            ## Append ops based on above
               Instr(8, 8, 0, 0, 5, 0),     ## Step34  #14: append idx 8(bot1) to idx 5 (assembled_list)
            ...
            ## Determine whether or not to iterate over again depending idx-i< len(madlibs_words)
               Instr(5, 3, 0, 0, 8, 0),     ## Step36  #9: Measure a length of index0 (madlibs_words) and assign it to idx 8(bot1)
               Instr(3, 9, 8, 2, 8, 1),   ## Step37  #3: Compare idx 9(idx-i) < idx 8(bot1) and assign result to idx 8(bot1)
