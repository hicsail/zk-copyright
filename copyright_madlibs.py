...
def main():
    ...
    scale = 200
    n_iter = 10000
    threshold = 10000000 # The program has to be performed within this (weight < )
    ...
    with PicoZKCompiler('irs/picozk_test', field=[p], options=['ram']):
        ...
        header = [
                ...
                ## IF madlibs_words[curr] == "_"
                    ...
                    Instr(6, sr3, di, di, di, di, di, sr2, di, 0),            ## Step2   #6: Set idx55 (temp-idx/reg3) of madlibs list to idx53 (reg2)
                    Instr(3, di, di,  0, us, sr2, di, sr2, di, 0),            ## Step3   #3: Compare idx53 (reg2) and "_" and assign result to idx53 (reg2)
                    Instr(4, di, sr2,  1, hc_size*4+3, di, di, di, di, 2),    ## Step4   #4: Cond jump to Step5/Step27 if true/false
                ]
        ...
        for i in range(hc_size, 0, -1):
                ...
                dynamic += [
                    Instr(3, di, di,  0, hc_size-i, sr4, di, sr3, di, 0),     #3: Compare idx57 (idx-k) == 0 ~ hc_size and assign result to idx55 (reg3)
                    Instr(4, di, sr3, hc_size*2-1,  1, di, di, di, di, 2)     #4: Cond jump to  +9/+1 if true/false
                ]
                # Instr(3, di, di,  0,  hc_size-5, sr4, di, sr3, di, 0),    ## Step5   #3: Compare idx57 (idx-k) == 0 and assign result to idx55 (reg3)
                # Instr(4, di, sr3,  9,  1, di, di, di, di, 2),             ## Step6   #4: Cond jump to  +9/+1 if true/false
                # Instr(3, di, di,  0,  hc_size-4, sr4, di, sr3, di, 0),    ## Step7   #3: Compare idx57 (idx-k) == 1 and assign result to idx55 (reg3)
                # Instr(4, di, sr3,  9,  1, di, di, di, di, 2),             ## Step8   #4: Cond jump to  +9/+1 if true/false
                # Instr(3, di, di,  0,  hc_size-3, sr4, di, sr3, di, 0),    ## Step9   #3: Compare idx57 (idx-k) == 2 and assign result to idx55 (reg3)
                # Instr(4, di, sr3,  9,  1, di, di, di, di, 2),             ## Step10  #4: Cond jump to  +9/+1 if true/false
                # Instr(3, di, di,  0,  hc_size-2, sr4, di, sr3, di, 0),    ## Step11  #3: Compare idx57 (idx-k) == 3 and assign result to idx55 (reg3)
                # Instr(4, di, sr3,  9,  1, di, di, di, di, 2),             ## Step12  #4: Cond jump to  +9/+1 if true/false
                # Instr(3, di, di,  0,  hc_size-1, sr4, di, sr3, di, 0),    ## Step13  #3: Compare idx57 (idx-k) == 4 and assign result to idx55 (reg3)
                # Instr(4, di, sr3,  9, hc_size*2+12, di, di, di, di, 2),   ## Step14  #4: Cond jump to  +9/+22 if true/false (sending to terminal if false as its error)


        for i in range(hc_size):
                dynamic += [
                    Instr(1, di, di, di, di, di,hcs[i], sr2, di, 0),
                    Instr(4, di, di,  hc_size*2-i*2-1, di, di, di, di, di, 0)
                    ]
                # ## k = 0: Set hc0 to reg2
                # Instr(1, di, di, di, di, di,hc0, sr2, di, 0),             ## Step15  #6: Set hc0 to idx53 (reg2)
                # Instr(4, di, di,  hc_size*2-1, di, di, di, di, di, 0),    ## Step16  #4: jump to Step25

                # ## k = 1: Set hc1 to reg2
                # Instr(1, di, di, di, di, di,hc1, sr2, di, 0),             ## Step17  #6: Set hc1 to idx53 (reg2)
                # Instr(4, di, di,  hc_size*2-3, di, di, di, di, di, 0),    ## Step18  #4: jump to Step25

                # ## k = 2: Set hc2 to reg2
                # Instr(1, di, di, di, di, di,hc2, sr2, di, 0),             ## Step19  #6: Set hc2 to idx53 (reg2)
                # Instr(4, di, di,  hc_size*2-5, di, di, di, di, di, 0),    ## Step20  #4: jump to Step25

                # ## k = 3: Set hc3 to reg2
                # Instr(1, di, di, di, di, di,hc3, sr2, di, 0),             ## Step21  #6: Set hc3 to idx53 (reg2)
                # Instr(4, di, di,  hc_size*2-7, di, di, di, di, di, 0),    ## Step22  #4: jump to Step25

                # ## k = 4: Set hc4 to reg2
                # Instr(1, di, di, di, di, di,hc4, sr2, di, 0),             ## Step23  #6: Set hc4 to idx53 (reg2)
                # Instr(4, di, di,  hc_size*2-9, di, di, di, di, di, 0),    ## Step24  #4: jump to Step25


        footter = [
                    ## (Common) Increment k by 1 and jump to Step30
                    Instr(2, di, di,  1, di, di, di, sr4, di, 0),             ## Step25  #2: add 1 to idx57 (idx-k)
                    Instr(4, di, di,  4, di, di, di, di, di, 0),              ## Step26  #4: jump to Step30

                    ## ELSE: Append from madlibs_list[idx-k] to res_list
                    Instr(5, sr1, di, di, di, di, di, sr3, di, 0),            ## Step27  #5: Copy idx51 (idx-i/reg1) to idx55 (temp-idx/reg3)
                    Instr(2, di, di, s_ml, di, di, di, sr3, di, 0),           ## Step28  #2: Add 17 to idx55 (temp-idx/reg3) = Shifting pointer idx-i to madlibs list by 17
                    Instr(6, sr3, di, di, di, di, di, sr2, di, 0),            ## Step29  #6: Set idx55 (temp-idx/reg3) of madlibs list to idx53 (reg2)
                    
                ## APPEND and INCREMENT
                    Instr(5, sr1, di, di, di, di, di, sr3, di, 0),            ## Step30  #5: Copy idx51 (idx-i/reg1) to idx55 (temp-idx/reg3)
                    Instr(2, di, di, s_rs, di, di, di, sr3, di, 0),           ## Step31  #2: Add 61 to idx55 (temp-idx/reg3) = Shifting pointer idx-i to res list by 61
                    Instr(7, di, di, sr2, di, di, di, di, sr3, 1),            ## Step32  #6: Set idx53 (reg2) to idx55 (temp-idx/reg3) of res list
                    Instr(2, di, di,  1, di, di, di, sr1, di, 0),             ## Step33  #2: Add 1 to idx 51 (idx-i)

                ## Determine whether or not to iterate over again depending idx-i< len(madlibs_words)
                    Instr(3, di, di, 2, lim, sr1, di, sr3, di, 0),            ## Step34  #3: Compare idx51 (idx-i) < p5 (X_len) and assign result to idx55 (reg3)
                    Instr(4, di, sr3, hc_size*4+15, 1, di, di, di, di, 1),    ## Step35  #4: Cond jump to -16/+1 if true/false
